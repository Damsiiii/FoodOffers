...
class PopeyesScraper(BaseScraper):
    ...
    def scrape_live(self):
        ...
        try:
            ...
            offers = intercept_api_deals("https://popeyes.com.lk", self.vendor_id, self.vendor_name)
        except Exception:
            pass
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    for img in soup.find_all("img"):
                        src = img.get("src") or ""
                        alt = img.get("alt") or ""
                        if "uploads" in src and ("Header_Banner" in src or "Drumstick" in src):
                            full_src = src if src.startswith("http") else "https://popeyes.com.lk/" + src.lstrip("/")
                            title = alt.strip() if alt and len(alt) > 3 else "Popeyes Louisiana Chicken Promotion"
                            offers.append({
                                "id": f"plk-promo-{idx}",
                                "title": title,
