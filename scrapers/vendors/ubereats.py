...
class UberEatsScraper(BaseScraper):
    ...
    def scrape_live(self) -> List[Dict[str, Any]]:
        ...
        from the official Uber Eats Colombo feed.
        """
        offers = []
        loc = {"address": "Colombo, Sri Lanka", "latitude": 6.9271, "longitude": 79.8612}
            ...
            with sync_playwright() as p:
                ...
                if payloads:
                    ...
                    def process_store(st_dict):
                        ...
                        # Extract promo text or discount matching
                        badge_txt = ""
                        endorsement = st_dict.get("endorsement", {})
                        if isinstance(endorsement, dict) and "text" in endorsement:
                            badge_txt = endorsement.get("text", "")

                        if any(w in raw_str.lower() for w in ["% off", "buy 1", "bogo", "save", "special offer", "top offer", "promo", "free item", "lkr", "discount"]):
                            seen.add(title)
                            ...
                            if pct_match:
                                ...
                                orig = 2400.0
                                disc = round(orig * (1 - disc_pct / 100))

                            offers.append({
                                "id": f"ubereats-feed-{idx}",
                                "title": f"{title} - {badge_txt}" if badge_txt else f"{title} Special Deal",
                                "description": f"Exclusive Uber Eats Sri Lanka deal at {title}",
                ...
                if not offers:
                    ...
                    for c in cards:
                        ...
                        if txt and any(w in txt.lower() for w in ["off", "buy", "bogo", "deal", "promo", "free", "save", "special", "top offer"]):
                            ...
                            title = lines[0] if lines else "Uber Eats Deal"
